...
# ==========================================
# 1. PAGE SETUP (Must be at the very top)
# ==========================================
st.set_page_config(
...
PROFESSIONAL_ROLES = [
    ...
    "Marketing Manager", "Sales Executive", "HR Manager", "Accountant / CA",
    "Civil Engineer", "Mechanical Engineer", "Electrical Engineer", "Doctor / Physician",
    "Nurse", "Teacher / Professor", "Graphic Designer", "UI/UX Designer",
    "Operations Manager", "Supply Chain Manager", "Customer Support Lead",
    "Administrative Officer", "Legal Associate", "Architect", "Content Writer",
    "Digital Marketing Specialist", "DevOps Engineer", "Cybersecurity Analyst", "Other"
]
...
METRIC_OPTIONS = [
    "Below 10th", "10th / SSC", "12th / HSC", "Diploma (after 10th)",
    "Diploma (after 12th)", "ITI", "Not applicable"
]

COLLEGE_DEGREES = [
    "None / Not applicable", "B.A.", "B.Com", "B.Sc", "B.Tech / B.E.", "BBA", "BCA",
    "B.Pharm", "B.Arch", "LLB", "MBBS", "BDS", "B.Ed", "M.A.", "M.Com", "M.Sc",
    "M.Tech / M.E.", "MBA", "MCA", "M.Pharm", "LLM", "MD / MS", "Ph.D",
    "CA (Chartered Accountant)", "CS (Company Secretary)", "Other"
]

INDUSTRY_TYPES = [
    "Information Technology (IT)", "Healthcare & Pharmaceuticals", "Banking & Financial Services",
    "Manufacturing", "Retail & E-commerce", "Education & EdTech", "Real Estate & Construction",
    "Telecommunications", "Media & Entertainment", "Hospitality & Tourism", "Logistics & Transportation",
    "Energy & Utilities", "Agriculture & Agri-business", "Government / Public Sector",
    "Consulting & Professional Services", "Automotive", "FMCG", "Other"
]
...
def build_prompt(
    role: str, total_experience: int, age: int, state: str, district: str,
    gender: str, metric_school: str, college_degree: str, extra_certificates: str,
    past_company: str, past_years: Optional[int], past_role: str,
    expected_min: Optional[int], expected_max: Optional[int],
    industry_type: str, industry_description: str,
) -> str:
...
# Hero Section (Title and Animation)
...
with col_text:
    ...
    # Custom HTML Button that scrolls down to the form
    st.markdown(
        """
        <a href="#fill-your-details" style="text-decoration: none;">
            <div style="background-color: #FF4B4B; color: white; padding: 10px 20px; border-radius: 5px; text-align: center; display: inline-block; font-weight: bold; margin-top: 10px;">
                👇 JUMP TO FORM
            </div>
        </a>
        """,
...
with col_anim:
    ...
    if lottie_anim:
        st_lottie(lottie_anim, height=300, key="salary_animation")

st.divider()

# ==========================================
# 5. THE FORM (Prevents reloading!)
# ==========================================
# This header acts as the anchor for the "Jump to Form" button above
st.header("📝 Fill Your Details", anchor="fill-your-details")
...
with st.form("salary_form"):
    ...
    with left_col:
        ...
        total_experience = st.slider("📊 Total Experience (Job)", min_value=0, max_value=40, value=0)
    ...
    with right_col:
        ...
        extra_certificates = st.text_input("🎓 Extra Certificates", placeholder="e.g. AWS, PMP")
        ...
        with sal_col1:
            expected_min = st.number_input("💵 Min", min_value=0, value=0, step=50_000, format="%d")
        with sal_col2:
            expected_max = st.number_input("💵 Max", min_value=0, value=0, step=50_000, format="%d")

    # Past job expander still safely inside the form
    with st.expander("🏢 Past Job Work Place", expanded=False):
        ...
        with past_col2:
            past_years = st.number_input("Years there", min_value=0, max_value=40, value=0, step=1, key="past_years")
        with past_col3:
            past_role = st.text_input("Post/Role", key="past_role")

    # The single, final submit button at the bottom of the form
    submit_button = st.form_submit_button("PREDICT SALARY", type="primary", use_container_width=True)
    ...
    if not district.strip():
        ...
        try:
            with st.spinner("Analyzing market data and calculating optimal salary..."):
                prompt = build_prompt(
                    role=role, total_experience=total_experience, age=int(age), state=state,
                    district=district.strip(), gender=gender, metric_school=metric_school,
                    college_degree=college_degree, extra_certificates=extra_certificates,
                    past_company=past_company, past_years=int(past_years) if past_years else None,
                    past_role=past_role, expected_min=int(expected_min) if expected_min else None,
                    expected_max=int(expected_max) if expected_max else None,
                    industry_type=industry_type, industry_description=industry_description,
                )
